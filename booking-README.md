...
- **`booking-horizon.sql`**：预约时间上限（只能约到下周，下下周及以后在数据库层面拦截）
...
> · 每天 08:00–22:00，1 小时一个时段，先约先得；**只能约到下周**，下下周还没开放
...
**Q：能约下周以后的吗？**
不能。**最多约到下周（下周日 22:00）**，下下周一及以后在数据库层面拦截，
翻到下周后「下一周 →」按钮会变灰。每周一零点，窗口自动整体后移一周。
